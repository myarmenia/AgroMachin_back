        ...
        <div class="right-sectionChild">
            ...
            <div class="app">
                <div class="new_role">
                    ...
                    <form action="{{route('roles.update', $role->id)}}" method="POST" id="form" class="role_form">
                        @csrf
                        @method('patch')
                        <div class="role_block">
                            <input type="text" class="role_input @error('name') error_border @enderror" placeholder="Դեր" name="name" value="{{  $role->name }}" >
                            ...
                            <div class="input_checkbox">
                                ...
                                @foreach($permission as $value)
                                    <label for="">
                                        <input type="checkbox" name="permission[]" value="{{ $value->id }}" {{ in_array($value->id, $rolePermissions) ? 'checked' : false }}> {{ $value->name }}
                                    </label>
                                @endforeach
                                @error('permission')
                                    <div class="error_message">{{ $message }}</div>
